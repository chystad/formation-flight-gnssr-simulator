...
class RecorderFlusher(sysModel.SysModel):
    ...
    def UpdateState(self, CurrentSimNanos: int) -> None:
        ...
        # logging.debug(f"[{self.logTag}] RSS = {proc.memory_info().rss / 1e9:.2f}")
    ...
    def flush(self, CurrentSimNanos: int | None = None) -> None:
        ...
        # logging.debug(f"[{self.logTag}] RSS = {proc.memory_info().rss / 1e9:.2f} GB after flush, but without writing to file")
